...
static void UpdateCachedName(nsIMsgDBHdr *aHdr,
...
void
nsMsgDBView::InitializeAtomsAndLiterals()
{
  ...
  if (!mMessengerStringBundle)
  {
    ...
    if (sBundleService)
      res = sBundleService->CreateBundle(propertyURL,
                                         getter_AddRefs(mMessengerStringBundle));
  }
...
nsresult
nsMsgDBView::GetPrefLocalizedString(const char *aPrefName,
                                    nsString& aResult)
{
  nsresult rv = NS_OK;
...
static nsresult
GetDisplayNameInAddressBook(const nsACString& emailAddress,
...
static void
UpdateCachedName(nsIMsgDBHdr *aHdr,
                 const char *header_field,
                 const nsAString& newName)
{
...
nsresult
nsMsgDBView::FetchAuthor(nsIMsgDBHdr * aHdr,
                         nsAString &aSenderString)
{
...
nsresult
nsMsgDBView::FetchAccount(nsIMsgDBHdr * aHdr, nsAString& aAccount)
{
...
nsresult
nsMsgDBView::FetchRecipients(nsIMsgDBHdr * aHdr,
...
nsresult
nsMsgDBView::FetchSubject(nsIMsgDBHdr * aMsgHdr,
                          uint32_t aFlags,
                          nsAString &aValue)
{
...
// of the Date: date.
nsresult
nsMsgDBView::FetchDate(nsIMsgDBHdr * aHdr,
                       ...
                       bool rcvDate)
{
  ...
  // if the message is from the last week, show the day of the week (Mon 3:15 pm).
...
nsresult
nsMsgDBView::FetchStatus(uint32_t aFlags,
                         nsAString &aStatusString)
{
...
nsresult
nsMsgDBView::FetchSize(nsIMsgDBHdr * aHdr,
                       nsAString &aSizeString)
{
  nsresult rv;
...
nsresult
nsMsgDBView::FetchPriority(nsIMsgDBHdr *aHdr,
...
nsresult
nsMsgDBView::FetchKeywords(nsIMsgDBHdr *aHdr,
...
nsresult
nsMsgDBView::FetchRowKeywords(nsMsgViewIndex aRow,
                              nsIMsgDBHdr *aHdr,
                              nsACString &keywordString)
{
...
nsresult
nsMsgDBView::FetchTags(nsIMsgDBHdr *aHdr,
                       nsAString &aTagString)
{
  NS_ENSURE_ARG_POINTER(aHdr);
  nsresult rv = NS_OK;
  if (!mTagService)
...
nsresult
nsMsgDBView::FetchLabel(nsIMsgDBHdr *aHdr,
                        nsAString &aLabelString)
{
  nsresult rv = NS_OK;
  nsMsgLabelValue label = 0;
...
nsresult
nsMsgDBView::SaveAndClearSelection(nsMsgKey *aCurrentMsgKey,
...
nsresult
nsMsgDBView::RestoreSelection(nsMsgKey aCurrentMsgKey,
                              nsTArray<nsMsgKey> &aMsgKeyArray)
{
  // We don't do anything on nested Save / Restore calls.
  m_saveRestoreSelectionDepth--;
...
nsresult
nsMsgDBView::GenerateURIForMsgKey(nsMsgKey aMsgKey,
                                  nsIMsgFolder *folder,
                                  nsACString & aURI)
{
...
nsMsgDBView::SelectionChanged()
{
  ...
  if (!summaryStateChanged &&
      ...
      commandsNeedDisablingBecauseOfSelection == mCommandsNeedDisablingBecauseOfSelection &&
      enableGoForward == mGoForwardEnabled &&
      enableGoBack == mGoBackEnabled)
  {
...
nsresult
nsMsgDBView::GetSelectedIndices(nsMsgViewIndexArray& selection)
{
  if (mTreeSelection)
  {
    ...
    for (int32_t i = 0; i < selectionCount; i++)
    {
      ...
      if (startRange >= 0 && startRange < viewSize)
      {
        for (int32_t rangeIndex = startRange;
             rangeIndex <= endRange && rangeIndex < viewSize;
             rangeIndex++)
        {
...
NS_IMETHODIMP
nsMsgDBView::IsContainer(int32_t index,
                         bool *_retval)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
NS_IMETHODIMP
nsMsgDBView::IsContainerOpen(int32_t index,
                             bool *_retval)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
NS_IMETHODIMP
nsMsgDBView::IsContainerEmpty(int32_t index,
                              bool *_retval)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
NS_IMETHODIMP
nsMsgDBView::IsSeparator(int32_t index,
                         bool *_retval)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
NS_IMETHODIMP
nsMsgDBView::GetParentIndex(int32_t rowIndex,
                            int32_t *_retval)
{
  *_retval = -1;
...
NS_IMETHODIMP
nsMsgDBView::HasNextSibling(int32_t rowIndex,
                            int32_t afterIndex,
                            bool *_retval)
{
  *_retval = false;
...
NS_IMETHODIMP
nsMsgDBView::GetLevel(int32_t index,
                      int32_t *_retval)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
nsresult
nsMsgDBView::GetMsgHdrForViewIndex(nsMsgViewIndex index,
                                   nsIMsgDBHdr **msgHdr)
{
  nsresult rv = NS_OK;
  if (!IsValidIndex(index))
...
void
nsMsgDBView::InsertMsgHdrAt(nsMsgViewIndex index,
                            nsIMsgDBHdr *hdr,
                            nsMsgKey msgKey,
                            uint32_t flags,
                            uint32_t level)
...
void
nsMsgDBView::SetMsgHdrAt(nsIMsgDBHdr *hdr,
...
nsresult
nsMsgDBView::GetFolderForViewIndex(nsMsgViewIndex index,
...
nsresult
nsMsgDBView::GetDBForViewIndex(nsMsgViewIndex index,
                               nsIMsgDatabase **db)
{
...
void
nsMsgDBView::RememberDeletedMsgHdr(nsIMsgDBHdr *msgHdr)
{
  nsCString messageId;
...
bool
nsMsgDBView::WasHdrRecentlyDeleted(nsIMsgDBHdr *msgHdr)
{
  nsCString messageId;
  msgHdr->GetMessageId(getter_Copies(messageId));
...
NS_IMETHODIMP
nsMsgDBView::PerformAction(const char16_t *action)
{
  return NS_OK;
}
...
NS_IMETHODIMP
nsMsgDBView::Open(nsIMsgFolder *folder,
...
NS_IMETHODIMP
nsMsgDBView::Init(nsIMessenger * aMessengerInstance,
...
int
CompareViewIndices (const void *v1,
                    const void *v2,
                    void *)
{
...
NS_IMETHODIMP
nsMsgDBView::GetSelectedMsgHdrs(uint32_t *aLength,
                                nsIMsgDBHdr ***aResult)
{
  nsresult rv;
...
NS_IMETHODIMP
nsMsgDBView::GetURIForViewIndex(nsMsgViewIndex index,
                                nsACString &result)
{
  nsresult rv;
...
// cross-folder thread.
nsresult
nsMsgDBView::ListCollapsedChildren(nsMsgViewIndex viewIndex,
...
nsresult
nsMsgDBView::GetHeadersFromSelection(uint32_t *indices,
                                     uint32_t numIndices,
                                     nsIMutableArray *messageArray)
{
  nsresult rv = NS_OK;
  ...
  for (uint32_t index = 0;
       index < (nsMsgViewIndex) numIndices && NS_SUCCEEDED(rv);
       index++)
  {
...
nsresult
nsMsgDBView::CopyMessages(nsIMsgWindow *window,
                          nsMsgViewIndex *indices,
                          int32_t numIndices,
                          bool isMove,
                          nsIMsgFolder *destFolder)
{
  ...
  return copyService->CopyMessages(m_folder /* source folder */,
                                   messageArray,
                                   destFolder,
                                   isMove,
                                   nullptr /* listener */,
...
nsresult
nsMsgDBView::ApplyCommandToIndicesWithFolder(nsMsgViewCommandTypeValue command,
                                             nsMsgViewIndex* indices,
                                             int32_t numIndices,
                                             nsIMsgFolder *destFolder)
{
                                   ...
                                   int32_t numIndices)
{
  ...
  if (thisIsImapFolder)
  {
    ...
    switch (command)
    {
      ...
      case nsMsgViewCommandType::unjunk:
      {
        ...
        if (msgFlags & nsMsgMessageFlags::IMAPDeleted)
          imapFolder->StoreImapFlags(kImapMsgDeletedFlag,
                                     false,
                                     imapUids.Elements(),
                                     imapUids.Length(),
                                     nullptr);
    ...
    {
      imapFolder->StoreImapFlags(flags,
                                 addFlags,
                                 imapUids.Elements(),
                                 imapUids.Length(),
                                 nullptr);
...
nsresult
nsMsgDBView::RemoveByIndex(nsMsgViewIndex index)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
nsresult
nsMsgDBView::DeleteMessages(nsIMsgWindow *window,
                            nsMsgViewIndex *indices,
                            int32_t numIndices,
                            bool deleteStorage)
{
...
nsresult
nsMsgDBView::DownloadForOffline(nsIMsgWindow *window,
                                nsMsgViewIndex *indices,
                                int32_t numIndices)
{
  nsresult rv = NS_OK;
  nsCOMPtr<nsIMutableArray> messageArray(do_CreateInstance(NS_ARRAY_CONTRACTID));
  for (nsMsgViewIndex index = 0;
       index < (nsMsgViewIndex) numIndices;
       index++)
  {
...
nsresult
nsMsgDBView::DownloadFlaggedForOffline(nsIMsgWindow *window)
{
  nsresult rv = NS_OK;
  nsCOMPtr<nsIMutableArray> messageArray(do_CreateInstance(NS_ARRAY_CONTRACTID));
  nsCOMPtr <nsISimpleEnumerator> enumerator;
...
nsresult
nsMsgDBView::ToggleReadByIndex(nsMsgViewIndex index)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
nsresult
nsMsgDBView::SetReadByIndex(nsMsgViewIndex index, bool read)
{
  nsresult rv;
...
nsresult
nsMsgDBView::SetThreadOfMsgReadByIndex(nsMsgViewIndex index,
...
nsresult
nsMsgDBView::SetFlaggedByIndex(nsMsgViewIndex index,
                               bool mark)
{
  nsresult rv;
...
nsresult
nsMsgDBView::SetMsgHdrJunkStatus(nsIJunkMailPlugin *aJunkPlugin,
                                 nsIMsgDBHdr *aMsgHdr,
                                 nsMsgJunkStatus aNewClassification)
{
    ...
    rv = aMsgHdr->GetStringProperty("junkscoreorigin", getter_Copies(oldOriginStr));
    ...
    nsCOMPtr<nsIMsgWindow> msgWindow(do_QueryReferent(mMsgWindowWeak));
    rv = aJunkPlugin->SetMessageClassification(uri.get(),
...
nsresult
nsMsgDBView::GetFolderFromMsgURI(const char *aMsgURI,
                                 nsIMsgFolder **aFolder)
{
  NS_IF_ADDREF(*aFolder = m_folder);
  return NS_OK;
...
nsMsgDBView::PerformActionsOnJunkMsgs(bool msgsAreJunk)
{
  ...
  if (moveMessages)
  {
    ...
    for (nsMsgViewIndex junkIndex = 0;
         !junkedMsgSelected && junkIndex < numJunkHdrs;
         junkIndex++)
    {
...
int
nsMsgDBView::FnSortIdKey(const void *pItem1,
                         const void *pItem2,
                         void *privateData)
{
  int32_t retVal = 0;
  ...
  mozilla::DebugOnly<nsresult> rv = db->CompareCollationKeys((*p1)->dword,
                                                             (*p1)->key,
                                                             (*p2)->dword,
                                                             (*p2)->key,
                                                             &retVal);
  ...
  if (sortInfo->view->m_secondarySort == nsMsgViewSortType::byId)
    ...
    return sortInfo->view->SecondarySort((*p1)->id,
                                         (*p1)->folder,
                                         (*p2)->id,
                                         (*p2)->folder,
                                         sortInfo);
...
int
nsMsgDBView::FnSortIdKeyPtr(const void *pItem1,
                            const void *pItem2,
                            void *privateData)
{
  int32_t retVal = 0;
...
int
nsMsgDBView::FnSortIdUint32(const void *pItem1,
                            const void *pItem2,
                            void *privateData)
{
...
// values must be 4 bytes aligned. Don't break this when modifying the constants.
...
nsresult
nsMsgDBView::GetStatusSortValue(nsIMsgDBHdr *msgHdr,
                                uint32_t *result)
{
...
nsresult
nsMsgDBView::EncodeColumnSort(nsString &columnSortString)
{
  for (uint32_t i = 0; i < m_sortColumns.Length(); i++)
  {
    MsgViewSortColumnInfo &sortInfo = m_sortColumns[i];
...
nsresult
nsMsgDBView::DecodeColumnSort(nsString &columnSortString)
{
...
nsresult
nsMsgDBView::GetCollationKey(nsIMsgDBHdr *msgHdr,
                             nsMsgViewSortTypeValue sortType,
                             uint8_t **result,
                             uint32_t *len,
                             nsIMsgCustomColumnHandler* colHandler)
{
  ...
  switch (sortType)
  {
    ...
    case nsMsgViewSortType::byTags:
    {
      ...
      rv = (sortType == nsMsgViewSortType::byAccount) ? FetchAccount(msgHdr, str) :
...
nsresult
nsMsgDBView::GetLocationCollationKey(nsIMsgDBHdr *msgHdr,
                                     uint8_t **result,
                                     uint32_t *len)
{
  nsCOMPtr <nsIMsgFolder> folder;
...
nsresult
nsMsgDBView::SaveSortInfo(nsMsgViewSortTypeValue sortType,
                          nsMsgViewSortOrderValue sortOrder)
{
  if (m_viewFolder)
  {
...
void
nsMsgDBView::EnsureCustomColumnsValid()
{
  if (!m_sortColumns.Length())
    return;
...
int32_t  nsMsgDBView::SecondarySort(nsMsgKey key1,
                                    nsISupports *supports1,
                                    nsMsgKey key2,
                                    nsISupports *supports2,
...
void
nsMsgDBView::FreeAll(nsTArray<void*> *ptrs)
{
  int32_t i;
...
nsMsgViewIndex
nsMsgDBView::GetIndexOfFirstDisplayedKeyInThread(nsIMsgThread *threadHdr,
                                                 bool allowDummy)
{
...
nsresult
nsMsgDBView::GetFirstMessageHdrToDisplayInThread(nsIMsgThread *threadHdr,
                                                 nsIMsgDBHdr **result)
{
  nsresult rv;
...
nsMsgViewIndex
nsMsgDBView::ThreadIndexOfMsg(nsMsgKey msgKey,
...
nsMsgViewIndex
nsMsgDBView::GetThreadIndex(nsMsgViewIndex msgIndex)
{
...
nsMsgKey
nsMsgDBView::GetKeyOfFirstMsgInThread(nsMsgKey key)
{
...
NS_IMETHODIMP
nsMsgDBView::GetKeyAt(nsMsgViewIndex index,
                      nsMsgKey *result)
{
...
NS_IMETHODIMP
nsMsgDBView::GetFlagsAt(nsMsgViewIndex aIndex,
                        uint32_t *aResult)
{
...
NS_IMETHODIMP
nsMsgDBView::GetMsgHdrAt(nsMsgViewIndex aIndex,
                         nsIMsgDBHdr **aResult)
{
  NS_ENSURE_ARG(aResult);
  if (!IsValidIndex(aIndex))
...
nsMsgViewIndex
nsMsgDBView::FindHdr(nsIMsgDBHdr *msgHdr,
...
nsMsgViewIndex
nsMsgDBView::FindKey(nsMsgKey key,
                     bool expand)
{
  nsMsgViewIndex retIndex = nsMsgViewIndex_None;
  ...
  if (key != nsMsgKey_None &&
      (retIndex == nsMsgViewIndex_None || m_flags[retIndex] & MSG_VIEW_FLAG_DUMMY) &&
      expand && m_db)
  {
...
nsresult
nsMsgDBView::GetThreadCount(nsMsgViewIndex index,
...
int32_t
nsMsgDBView::CountExpandedThread(nsMsgViewIndex index)
{
...
nsresult
nsMsgDBView::ExpansionDelta(nsMsgViewIndex index,
...
nsresult
nsMsgDBView::ToggleExpansion(nsMsgViewIndex index,
                             uint32_t *numChanged)
{
  nsresult rv;
...
nsresult
nsMsgDBView::ExpandByIndex(nsMsgViewIndex index,
...
nsresult
nsMsgDBView::OnNewHeader(nsIMsgDBHdr *newHdr,
                         nsMsgKey aParentKey,
                         bool /*ensureListed*/)
{
  nsresult rv = NS_OK;
...
nsMsgViewIndex
nsMsgDBView::GetInsertIndexHelper(nsIMsgDBHdr *msgHdr,
...
nsMsgViewIndex
nsMsgDBView::GetInsertIndex(nsIMsgDBHdr *msgHdr)
{
  if (!GetSize())
    return 0;
...
nsresult
nsMsgDBView::AddHdr(nsIMsgDBHdr *msgHdr,
...
bool
nsMsgDBView::WantsThisThread(nsIMsgThread * /*threadHdr*/)
{
...
nsMsgViewIndex
nsMsgDBView::FindParentInThread(nsMsgKey parentKey,
...
nsresult
nsMsgDBView::ListIdsInThreadOrder(nsIMsgThread *threadHdr,
                                  nsMsgKey parentKey,
                                  uint32_t level,
                                  nsMsgViewIndex *viewIndex,
                                  uint32_t *pNumListed)
...
bool
nsMsgDBView::InsertEmptyRows(nsMsgViewIndex viewIndex,
                             int32_t numRows)
{
...
void
nsMsgDBView::RemoveRows(nsMsgViewIndex viewIndex,
                        int32_t numRows)
{
...
NS_IMETHODIMP
nsMsgDBView::InsertTreeRows(nsMsgViewIndex aIndex,
                            uint32_t aNumRows,
                            nsMsgKey aKey,
...
NS_IMETHODIMP
nsMsgDBView::RemoveTreeRows(nsMsgViewIndex aIndex,
                            uint32_t aNumRows)
{
...
nsresult
nsMsgDBView::ListIdsInThread(nsIMsgThread *threadHdr,
                             nsMsgViewIndex startOfThreadViewIndex,
                             uint32_t *pNumListed)
{
...
int32_t
nsMsgDBView::FindLevelInThread(nsIMsgDBHdr *msgHdr,
...
void
nsMsgDBView::InitEntryInfoForIndex(nsMsgViewIndex i,
                                   IdKeyPtr &EntryInfo)
{
...
nsresult
nsMsgDBView::ListUnreadIdsInThread(nsIMsgThread *threadHdr,
                                   nsMsgViewIndex startOfThreadViewIndex,
                                   uint32_t *pNumListed)
{
  NS_ENSURE_ARG(threadHdr);
...
NS_IMETHODIMP
nsMsgDBView::OnHdrFlagsChanged(nsIMsgDBHdr *aHdrChanged,
                               uint32_t aOldFlags,
                               uint32_t aNewFlags,
...
NS_IMETHODIMP
nsMsgDBView::OnHdrDeleted(nsIMsgDBHdr *aHdrChanged,
                          nsMsgKey aParentKey,
                          int32_t aFlags,
                          nsIDBChangeListener *aInstigator)
{
...
NS_IMETHODIMP
nsMsgDBView::OnHdrAdded(nsIMsgDBHdr *aHdrChanged,
                        nsMsgKey aParentKey,
                        int32_t aFlags,
                        nsIDBChangeListener *aInstigator)
{
...
NS_IMETHODIMP
nsMsgDBView::OnParentChanged (nsMsgKey aKeyChanged,
                              nsMsgKey oldParent,
                              nsMsgKey newParent,
                              nsIDBChangeListener *aInstigator)
{
...
NS_IMETHODIMP
nsMsgDBView::OnEvent(nsIMsgDatabase *aDB,
                     const char *aEvent)
{
...
NS_IMETHODIMP
nsMsgDBView::OnReadChanged(nsIDBChangeListener *aInstigator)
{
  return NS_OK;
}
...
NS_IMETHODIMP
nsMsgDBView::GetSuppressChangeNotifications(bool * aSuppressChangeNotifications)
{
...
NS_IMETHODIMP
nsMsgDBView::NoteChange(nsMsgViewIndex firstLineChanged,
                        int32_t numChanged,
...
void
nsMsgDBView::NoteStartChange(nsMsgViewIndex firstlineChanged,
                             int32_t numChanged,
                             nsMsgViewNotificationCodeValue changeType)
{
}
...
void
nsMsgDBView::NoteEndChange(nsMsgViewIndex firstlineChanged,
                           int32_t numChanged,
                           nsMsgViewNotificationCodeValue changeType)
{
...
nsresult
nsMsgDBView::PersistFolderInfo(nsIDBFolderInfo **dbFolderInfo)
{
...
nsresult
nsMsgDBView::MarkThreadOfMsgRead(nsMsgKey msgId,
                                 nsMsgViewIndex msgIndex,
...
nsresult
nsMsgDBView::MarkThreadRead(nsIMsgThread *threadHdr,
                            nsMsgViewIndex threadIndex,
                            nsTArray<nsMsgKey> &idsMarkedRead,
                            bool bRead)
{
...
bool
nsMsgDBView::AdjustReadFlag(nsIMsgDBHdr *msgHdr,
                            uint32_t *msgFlags)
{
...
NS_IMETHODIMP
nsMsgDBView::ViewNavigate(nsMsgNavigationTypeValue motion,
                          nsMsgKey *pResultKey,
                          ...
                          bool wrap)
{
  ...
  return nsMsgDBView::NavigateFromPos(motion,
                                      startIndex,
                                      pResultKey,
                                      pResultIndex,
                                      pThreadIndex,
...
nsresult
nsMsgDBView::NavigateFromPos(nsMsgNavigationTypeValue motion,
                             nsMsgViewIndex startIndex,
                             nsMsgKey *pResultKey,
                             nsMsgViewIndex *pResultIndex,
                             nsMsgViewIndex *pThreadIndex,
                             bool wrap)
{
  ...
  switch (motion)
  {
    ...
    case nsMsgNavigationType::previousMessage:
      *pResultIndex =
        (startIndex != nsMsgViewIndex_None && startIndex > 0) ? startIndex - 1 : 0;
      *pResultKey = m_keys[*pResultIndex];
      break;
    ...
    case nsMsgNavigationType::forward:
    {
      ...
      int32_t relPos = (motion == nsMsgNavigationType::forward) ?
                         1 :
                         (m_currentlyDisplayedMsgKey != nsMsgKey_None) ? -1 : 0;
      int32_t curPos;
      nsresult rv;
      nsCOMPtr<nsIMessenger> messenger(do_QueryReferent(mMessengerWeak, &rv));
      NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
nsMsgDBView::NavigateStatus(nsMsgNavigationTypeValue motion,
                            bool *_retval)
{
  NS_ENSURE_ARG_POINTER(_retval);
...
nsresult
nsMsgDBView::FindNextFlagged(nsMsgViewIndex startIndex,
                             nsMsgViewIndex *pResultIndex)
{
...
nsresult
nsMsgDBView::FindFirstNew(nsMsgViewIndex *pResultIndex)
{
  if (m_db)
  {
...
nsresult
nsMsgDBView::FindPrevUnread(nsMsgKey startKey,
                            nsMsgKey *pResultKey,
                            nsMsgKey *resultThreadId)
{
  ...
  *pResultKey = nsMsgKey_None;
  ...
  NS_ASSERTION(!(*pResultKey != nsMsgKey_None &&
               resultThreadId &&
               *resultThreadId == nsMsgKey_None), "fix this");
  return rv;
}

nsresult
nsMsgDBView::FindFirstFlagged(nsMsgViewIndex *pResultIndex)
{
  return FindNextFlagged(0, pResultIndex);
}

nsresult
nsMsgDBView::FindPrevFlagged(nsMsgViewIndex startIndex,
                             nsMsgViewIndex *pResultIndex)
{
  nsMsgViewIndex curIndex;
...
nsresult
nsMsgDBView::OrExtraFlag(nsMsgViewIndex index,
                         uint32_t orflag)
{
  uint32_t  flag;
  if (!IsValidIndex(index))
...
nsresult
nsMsgDBView::AndExtraFlag(nsMsgViewIndex index,
                          uint32_t andflag)
{
  uint32_t  flag;
  if (!IsValidIndex(index))
...
nsresult
nsMsgDBView::SetExtraFlag(nsMsgViewIndex index,
                          uint32_t extraflag)
{
  if (!IsValidIndex(index))
    return NS_MSG_INVALID_DBVIEW_INDEX;
...
nsresult
nsMsgDBView::ToggleIgnored(nsMsgViewIndex * indices,
                           int32_t numIndices,
                           nsMsgViewIndex *resultIndex,
...
nsresult
nsMsgDBView::ToggleMessageKilled(nsMsgViewIndex * indices,
                                 int32_t numIndices,
                                 nsMsgViewIndex *resultIndex,
                                 bool *resultToggleState)
{
...
nsMsgViewIndex
nsMsgDBView::GetThreadFromMsgIndex(nsMsgViewIndex index,
...
nsresult
nsMsgDBView::ToggleWatched(nsMsgViewIndex* indices,
                           int32_t numIndices)
{
  nsCOMPtr <nsIMsgThread> thread;
...
nsresult
nsMsgDBView::SetThreadIgnored(nsIMsgThread *thread,
                              nsMsgViewIndex threadIndex,
                              bool ignored)
{
...
nsresult
nsMsgDBView::SetSubthreadKilled(nsIMsgDBHdr *header,
                                nsMsgViewIndex msgIndex,
                                bool ignored)
{
  if (!IsValidIndex(msgIndex))
...
nsresult
nsMsgDBView::SetThreadWatched(nsIMsgThread *thread,
                              nsMsgViewIndex index,
                              bool watched)
{
  if (!IsValidIndex(index))
...
bool
nsMsgDBView::OfflineMsgSelected(nsMsgViewIndex * indices,
                                int32_t numIndices)
{
  nsCOMPtr <nsIMsgLocalMailFolder> localFolder = do_QueryInterface(m_folder);
  if (localFolder)
...
bool
nsMsgDBView::NonDummyMsgSelected(nsMsgViewIndex * indices,
                                 int32_t numIndices)
{
  bool includeCollapsedMsgs = OperateOnMsgsInCollapsedThreads();
...
nsresult
nsMsgDBView::AdjustRowCount(int32_t rowCountBeforeSort,
...
nsresult
nsMsgDBView::GetImapDeleteModel(nsIMsgFolder *folder)
{
   nsresult rv = NS_OK;
   nsCOMPtr <nsIMsgIncomingServer> server;
   // For the search view.
...
NS_IMETHODIMP
nsMsgDBView::SelectFolderMsgByKey(nsIMsgFolder *aFolder,
                                  nsMsgKey aKey)
{
  NS_ENSURE_ARG_POINTER(aFolder);
...
nsresult
nsMsgDBView::CopyDBView(nsMsgDBView *aNewMsgDBView,
...
NS_IMETHODIMP
nsMsgDBView::FindIndexFromKey(nsMsgKey aMsgKey,
                              bool aExpand,
...
NS_IMETHODIMP
nsMsgDBView::FindIndexOfMsgHdr(nsIMsgDBHdr *aMsgHdr,
                               bool aExpand,
                               nsMsgViewIndex *aIndex)
{
...
static void
getDateFormatPref(nsIPrefBranch* _prefBranch,
...
void
nsMsgDBView::SetMRUTimeForFolder(nsIMsgFolder *folder)
{
  uint32_t seconds;
...
nsresult
nsMsgDBView::GetViewEnumerator(nsISimpleEnumerator **enumerator)
{
...
nsresult
nsMsgDBView::GetDBForHeader(nsIMsgDBHdr *msgHdr,
                            nsIMsgDatabase **db)
{
  nsCOMPtr<nsIMsgFolder> folder;
  nsresult rv = msgHdr->GetFolder(getter_AddRefs(folder));
 ...
 */
bool
nsMsgDBView::JunkControlsEnabled(nsMsgViewIndex aViewIndex)
{
