...
//  Assert.ok(aText.startsWith("Connection to server Mail for  timed out."));
  gGotAlert = true;
}
...
var tests = [
  ...
  function* compactOneFolder() {
    ...
    Assert.ok(!gStreamedHdr.flags & Ci.nsMsgMessageFlags.Offline);
  ...
  function* verifyNoOfflineMsg() {
    ...
    Assert.ok(!movedMsg.flags & Ci.nsMsgMessageFlags.Offline);
    } catch (ex) {dump(ex);}
    yield false;
    yield false;
  },
