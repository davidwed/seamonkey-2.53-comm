...
GeolocationPermissionPrompt.prototype = {
  ...
  get message() {
    let message;
    if (this.principal.URI.schemeIs("file")) {
      message = gNotificationBundle.GetStringFromName("geolocation.shareWithFile3");
    } else {
      let hostPort = "<>";
      try {
        hostPort = this.principal.URI.hostPort;
      } catch (ex) { }
      message = gNotificationBundle.formatStringFromName("geolocation.shareWithSite3",
                                                    [hostPort], 1);
    }
    return message;
  },
...
PersistentStoragePermissionPrompt.prototype = {
  ...
  get message() {
    ...
    return gNotificationBundle.formatStringFromName(
